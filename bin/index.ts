...
import { rankModels } from '../src/ranking.js';
import { PROJECT_TOKENS, ProjectType, getProjects, getAllModels } from '../src/contracts.js';
import { getModels } from '../src/models.js';
import { syncPrices } from '../src/openrouter.js';

const program = new Command();

program
  .name('vibe-budget')
  .description('CLI tool to estimate AI coding costs and compare models')
  .version('1.0.0');

program
  .command('scan')
  .description('Scan a project folder and estimate costs')
  .argument('[path]', 'Project path to scan', '.')
  .option('-t, --type <type>', 'Analysis type: debug, bug, fix, refactor, review, new', 'new')
  .option('-a, --args <args...>', 'Additional arguments for analysis type detection')
  .action(async (path: string, options: { type?: string; args?: string[] }) => {
    const args = [options.type || 'new', ...(options.args || [])];
    await scanCommand(path, args);
  });
...
program
  .command('prices')
  .description('View current model prices from OpenRouter')
...
function printEstimate(
  ...
  ranking: ReturnType<typeof rankModels>,
  verbose: boolean
): void {
  const boxWidth = 50;
